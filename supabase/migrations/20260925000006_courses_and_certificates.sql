-- Free Setu courses, and a "Setu Certified" badge for students who finish one.
-- Courses and lessons are fixed content in the app (src/data/mockCourses.ts), not a table:
-- only progress needs to be stored, as a list of finished lesson ids on the student.
--
-- Migrations 1-5 may already be applied, so they stay as they were. This file:
--   1. Adds completed_lessons on students and suggested_course_id on internships.
--   2. Replaces the demo seeder so a reset or a fresh database seeds sample course
--      progress and suggested courses too.
--   3. Backfills the new columns on a database that already has the old demo data.
--      Guarded by id and the column still being empty, so a student's or business's
--      own edits since are left alone.

alter table public.students add column if not exists completed_lessons text[] not null default '{}';
alter table public.internships add column if not exists suggested_course_id text;
...
as $fn$
begin
  insert into public.students (id, position, first_name, last_name, short_name, phone, college, course, year, town, skills, onboarded, college_verified, completed_lessons) values
    ('stu_priya', 1, 'Priya', 'Kulkarni', 'Priya K.', '+91 98220 41207', 'BCOM Arts & Commerce College', 'BCom', 'Final year', 'Nashik', array['Tally', 'Excel', 'Customer service'], true, true, array[]::text[]),
    ('stu_aman', 2, 'Aman', 'Shaikh', 'Aman S.', '+91 97630 55810', 'BCOM Arts & Commerce College', 'BCom', 'Final year', 'Nashik', array['Excel', 'Stock keeping'], true, true, array['lsn_exceltally_1']),
    ('stu_neha', 3, 'Neha', 'Patil', 'Neha P.', '+91 90110 76342', 'BCOM Arts & Commerce College', 'BBA', 'Final year', 'Nashik', array['Social media', 'Canva', 'Communication'], true, true, array[]::text[]),
    ('stu_rahul', 4, 'Rahul', 'Mane', 'Rahul M.', '+91 88056 19934', 'BCOM Arts & Commerce College', 'BCom', 'Final year', 'Nashik', array['Billing', 'Excel', 'Basic English'], true, true, array[]::text[]),
    ('stu_sneha', 5, 'Sneha', 'Rane', 'Sneha R.', '+91 98904 20573', 'BCOM Arts & Commerce College', 'BCom', 'Final year', 'Nashik', array['Tally', 'GST basics'], true, true, array['lsn_gstbilling_1', 'lsn_gstbilling_2', 'lsn_gstbilling_3']),
    ('stu_karan', 6, 'Karan', 'Deshpande', 'Karan D.', '+91 70200 88461', 'BCOM Arts & Commerce College', 'BCA', 'Final year', 'Nashik', array['Excel', 'Data entry', 'Customer calls'], true, true, array['lsn_digital_1', 'lsn_digital_2', 'lsn_digital_3']);
...
-- Backfill: give the three demo students their sample course progress, and the six
-- openings their suggested course, on a database that already ran migration 5.
-- Guarded so a student's or business's own edit since is left as they made it.
update public.students set completed_lessons = array['lsn_exceltally_1']
  where id = 'stu_aman' and completed_lessons = '{}'::text[];

update public.students set completed_lessons = array['lsn_gstbilling_1', 'lsn_gstbilling_2', 'lsn_gstbilling_3']
  where id = 'stu_sneha' and completed_lessons = '{}'::text[];

update public.students set completed_lessons = array['lsn_digital_1', 'lsn_digital_2', 'lsn_digital_3']
  where id = 'stu_karan' and completed_lessons = '{}'::text[];

update public.internships set suggested_course_id = 'course_excel_tally' where id = 'int_accounts' and suggested_course_id is null;
update public.internships set suggested_course_id = 'course_gst_billing' where id = 'int_billing' and suggested_course_id is null;
update public.internships set suggested_course_id = 'course_excel_tally' where id = 'int_operations' and suggested_course_id is null;
update public.internships set suggested_course_id = 'course_digital' where id = 'int_social' and suggested_course_id is null;
update public.internships set suggested_course_id = 'course_excel_tally' where id = 'int_inventory' and suggested_course_id is null;
update public.internships set suggested_course_id = 'course_digital' where id = 'int_digital' and suggested_course_id is null;
