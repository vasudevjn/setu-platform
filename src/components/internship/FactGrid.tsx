...
export function FactGrid({ internship, sme }: { internship: Internship; sme: SME }) {
  const facts = [
    ...
    { icon: Clock, label: 'Duration', value: `${internship.weeks} weeks · ${internship.hours}` },
    { icon: MapPin, label: 'Distance', value: `${sme.distanceKm} km from home` },
    { icon: GraduationCap, label: 'Credit', value: internship.countsForCredit ? 'Counts for AICTE credit' : 'Credit not confirmed' },
  ]
  return (
